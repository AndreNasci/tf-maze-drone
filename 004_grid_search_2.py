...
def main():
    ...
    if len(sys.argv) != 4:
        print("Usage: X Y -> X = Combination | Y = buffer | Z = fc ")
        return -1
    ...
    replay_buffer_max_length = buffer_options[buffer]  
    ...
    # LOGGING
    df_log = pd.DataFrame({'Step': step_log, 'Average Return': returns, '% Finished': finished, 'Crash Counter': crashed, 'Stuck Counter': crashed, 'Avg Steps/Episode': steped, 'Loss log': log_loss})
    df_log.to_csv(f"logs/06-hist-env/{description}-combination_{combination}-buffer_{buffer}-fc_{fc}.csv", index=None, header=True)

...
def main():
    ...
    session = TrainingSession("initiate", env_name, rewards[0], agent, collect_steps_per_iteration, 
    ...
    for combination in range(min_combination-1, min_combination):
        ...
        for run in range(min_run-1, max_run):
            ...
            # GENERATE TRAINING SESSION
            session = TrainingSession(f"10-comb-{combination+1}-run-{run+1}", env_name, rewards[combination], agent, collect_steps_per_iteration, 
                                    num_iterations, eval_interval, replay_buffer_max_length, num_eval_episodes)
            
            # TRAINING
            step_log, returns, finished, crashed, stucked, steped, log_loss, _, _= session.train()
            ...
            df_log.to_csv(f"logs/02-stuck-improving/10_comb-{combination+1}-run-{run+1}.csv", index=None, header=True)

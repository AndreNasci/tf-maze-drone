...
def main():
    ...
    for run in range(min_run-1, max_run):
        ...
        # TRAINING
        step_log, returns, finished, crashed, stucked, steped, log_loss, _, agent = session.train(without_wall_training=False, early_stop="crash")

        print(agent.train_step_counter)
        combination = min_combination
        #agent._optimizer.learning_rate = 0.0001

        
        # GENERATE SECOND TRAINING SESSION
        session = TrainingSession(description, maze_size, env_name, rewards[combination], agent, collect_steps_per_iteration, 
                                num_iterations, eval_interval, replay_buffer_max_length, num_eval_episodes)
        
        # TRAINING_2
        step_log_2, returns_2, finished_2, crashed_2, stucked_2, steped_2, log_loss_2, _, _ = session.train(without_wall_training=False, early_stop="stuck")

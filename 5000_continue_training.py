...
def main():
    ...
    if len(sys.argv) != 4:
        print("Usage: X Y -> X = Combination | Y = Checkpoint | Z = steps_per_iteration")
        return -1
    ...
    collect_steps_per_iteration = int(sys.argv[3]) # @param {type:"integer"}
    ...
    checkpoint_dir = f'./checkpoint/phase-2-{checkpoint_num}-stuck'
    ...
    for combination in range(min_combination-1, min_combination):
        ...
        for run in range(min_run-1, max_run):
            ...
            df_log.to_csv(f"logs/04-stateChange/{description}_checkpoint_{checkpoint_num}_comb-{combination+1}-run-{run+1}.csv", index=None, header=True)
            ...
            checkpoint_dir = f'./checkpoint/phase-3-{checkpoint_num - 1}-tuning'
            ...
            print("Checkpoint created in", checkpoint_dir)

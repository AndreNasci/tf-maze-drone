...
def main():
    ...
    for run in range(2):
        ...
        combination = 2 # da série de arquivos 6000
        collect_steps_per_iteration = int(sys.argv[3]) # 5
        ...
        agent._optimizer.learning_rate = 1e-4
        ...
        # LOGGING
        df_log_phase_4 = pd.DataFrame({'Step': step_log, 'Average Return': returns, '% Finished': finished, 'Crash Counter': crashed, 'Stuck Counter': stucked, 'Avg Steps/Episode': steped, 'Loss log': log_loss})
        ...
        df_log = pd.concat([df_log_phase_1, df_log_phase_2, df_log_phase_3, df_log_phase_4])
